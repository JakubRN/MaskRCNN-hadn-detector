...
if __name__ == '__main__':
    ...
    try:
        os.mkdir("exp_2")
        os.mkdir("exp_2/data")
        os.mkdir("exp_2/masks")
        os.mkdir("exp_2/masks/1")
        os.mkdir("exp_2/masks/2")
        os.mkdir("exp_2/masks/3")
        os.mkdir("exp_2/masks/4")
    except FileExistsError as identifier:
        print("directories already exist")

    maskrcnn_calculation = MaskRCNN_maniuplator(None)
    ...
    for index, data in enumerate(os.walk("data/")):
        ...
        for img in data[2]:
            ...
            result_feed = result_feed.transpose(2,1,0)
            with torch.no_grad():
                prediction = maskrcnn_calculation.model([torch.as_tensor(result_feed, dtype=torch.float32).to(device)])

            mask = prediction[0]['masks'].permute(1, 0, 3, 2)[0].cpu().detach().numpy()
            boxes = prediction[0]['boxes'].cpu().detach().numpy()
            # main_classificator.classify()
            imgdata[mask[0] > maskrcnn_calculation.mask_accuracy] = 255
            ...
            if c == ord('q'):
                ...
                label_ind = data[0].split('/')[-1]
                img_ind = img.split('.')[0]
                img_string = "exp_2/data/" +  label_ind + '_' + img_ind + ".png"
                mask_string = "exp_2/masks/1/" +  label_ind + '_' + img_ind + ".png"
                global_image_counter +=1
                copyfile(data[0] + "/" + img, img_string)
                cv2.imwrite(mask_string, (mask[0] > maskrcnn_calculation.mask_accuracy) * 255)